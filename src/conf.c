...
typedef enum {
    ...
    oPortalServHTTPPort,
    oPortalServPath,
    oPortalServIndexScriptPathFragment,
} OpCodes;
...
static const struct {
    ...
    "pshostname", oPortalServHostname}, {
    "pshttpport", oPortalServHTTPPort}, {
    "pspath", oPortalServPath}, {
    "indexscriptpathfragment", oPortalServIndexScriptPathFragment}, {
NULL, oBadOption},};
...
config_init(void)
{
    ...
    config.portal_server = NULL; //add by bgofwinter
...
/**@internal
*Parses portal server information
*/
static void
parse_portal_server(FILE *file, const char *filename, int *linenum)
{
    char *host = NULL,
        *path = NULL,
        *indexscriptpathfragment = NULL,
        line[MAX_BUF], *p1, *p2;
    int http_port, opcode;
    t_portal_serv *new = NULL;

    /* Defaults */
    path = safe_strdup(DEFAULT_PORTALSERVPATH);
    indexscriptpathfragment = safe_strdup(DEFAULT_PORTALSERVINDEXPATHFRAGMENT);
    http_port = DEFAULT_PORTALSERVPORT;
    ...
    while (memset(line, 0, MAX_BUF) && fgets(line, MAX_BUF - 1, file) && (strchr(line, '}') == NULL)) {
        ...
        if (strlen(p1) > 0) {
            ...
            *p2 = '\0';
            ...
            switch (opcode) {
            case oPortalServHostname:
                /* Coverity rightfully pointed out we could have duplicates here. */
                if (NULL != host)
                    free(host);
                host = safe_strdup(p2);
                break;
            case oPortalServPath:
                free(path);
                path = safe_strdup(p2);
                break;
            case oPortalServIndexScriptPathFragment:
                free(indexscriptpathfragment);
                indexscriptpathfragment = safe_strdup(p2);
                break;
            case oPortalServHTTPPort:
                http_port = atoi(p2);
                break;
            case oBadOption:
            default:
    ...
    debug(LOG_DEBUG, "Adding %s:%d %s to the portal server list", host, http_port, path);

    /* Allocate memory */
    new = safe_malloc(sizeof(t_portal_serv));

    /* Fill in struct */
    new->portalserv_hostname = host;
    new->portalserv_path = path;
    new->portalserv_index_script_path_fragment = indexscriptpathfragment;
    new->portalserv_http_port = http_port;

    /* add to config */
    if (config.portal_server == NULL) {
        config.portal_server = new;
    }
        ...
        if (p1 && p1[0] != '\0') {
            ...
            if ((strncmp(s, "#", 1)) != 0) {
                ...
                case oSSLUseSNI:
                    ...
                    break;
		case oPortalServer:
		    parse_portal_server(fd, filename, &linenum);
		    break;
                case oBadOption:
                    /* FALL THROUGH */
                default:
