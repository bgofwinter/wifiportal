...
iptables_fw_init(void)
{
    ...
    iptables_do_command("-t mangle -A " CHAIN_INCOMING " -j ACCEPT");//add by bgofwinter
...
iptables_fw_access(fw_access_t type, const char *ip, const char *mac, int tag)
{
    ...
    case FW_ACCESS_ALLOW:
        ...
        iptables_do_command("-t mangle -A " CHAIN_OUTGOING " -m mac --mac-source %s -j MARK --set-mark %d", mac, tag);
    ...
    case FW_ACCESS_DENY:
    	...
    	//modify by bgofwinter
        iptables_do_command("-t mangle -D " CHAIN_OUTGOING " -m mac --mac-source %s -j MARK --set-mark %d", mac, tag);
...
iptables_fw_counters_update(void)
{
    ...
    while (output && !(feof(output))) {
        //rc = fscanf(output, "%*s %llu %*s %*s %*s %*s %*s %*s %*s %*s %17[0-9a-zA-F.] %*s %*s %*s",
        rc = fscanf(output, "%*s %llu %*s %*s %*s %*s %*s %*s %*s %*s %17[0-9a-zA-F:] %*s %*s %*s",
		&counter, mac);//modify by bgofwinter
	...
	str_to_lower(mac);//add by bgofwinter
