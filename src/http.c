...
http_callback_404(httpd * webserver, request * r, int error_code)
{
    ...
    t_portal_serv *portal_server = config->portal_server;
    ...
    /*snprintf(tmp_url, (sizeof(tmp_url) - 1), "http://%s:%d%s%s%s",
             portal_server->portalserv_hostname,
	     portal_server->portalserv_http_port,
	     portal_server->portalserv_path,
	     portal_server->portalserv_index_script_path_fragment,
	     r->request.query);
    */
    ...
    if (!is_online()) {
        ...
        debug(LOG_INFO, "Captured %s requesting [%s] and re-directing them to portal server index page", r->clientAddr, url);
        //http_send_redirect_to_auth(r, urlFragment, "Redirect to login page");
	//modify by bgofwinter
        http_send_redirect_to_portal(r, urlFragment, "Redirect to portal server index page");
        free(urlFragment);
    }
    free(url);
}
...
void http_send_redirect_to_portal(request *r, const char *urlFragment, const char *text)
{
	s_config *config = config_get_config();
	t_portal_serv *portal_server = config->portal_server;
	int port = portal_server->portalserv_http_port;
	char *url = NULL;

	safe_asprintf(&url, "http://%s:%d%s%smac=%s",
		portal_server->portalserv_hostname,
		port,
		portal_server->portalserv_path,
		portal_server->portalserv_index_script_path_fragment,
		config->gw_id);
		//portal_server->portalserv_hostname, port, portal_server->portalserv_path, urlFragment);
	http_send_redirect(r, url, text);
...
http_callback_auth(httpd * webserver, request * r)
{
    ...
    httpVar *param_mac = httpdGetVariableByName(r, "mac");

    if ((token = httpdGetVariableByName(r, "token"))) {
	if (param_mac) {

    	    str_to_lower(param_mac->value);

            /* We got station MAC address from url's param*/
            LOCK_CLIENT_LIST();

            if ((client = client_list_find(r->clientAddr, param_mac->value)) == NULL) {
                debug(LOG_DEBUG, "New client for %s", r->clientAddr);
                client_list_add(r->clientAddr, param_mac->value, token->value);
            } else if (logout) {
